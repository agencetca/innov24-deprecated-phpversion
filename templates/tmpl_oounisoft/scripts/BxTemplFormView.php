...
class BxBaseFormView extends BxDolForm {
	...
	var $hasDatePicker = false;	/* Deano Datapicker Fix */
     ...
     */
    function getCode() {
		/* Deano Datapicker Fix - Start */
		$sReturn = ($this->sCode = $this->genForm());
		if($this->hasDatePicker == true) $GLOBALS['oSysTemplate']->addJs(array('deano.js'));
        return $sReturn;
		/* Deano Datapicker Fix - Start */
    }
...
BLAH;
    ...
    function genInfoIcon($sInfo) {
        ...
        $sImgUrl = getTemplateIcon('info.png');
...
class BxTemplFormView extends BxDolForm {
	var $hasDatePicker = false;	/* Deano Datapicker Fix */
...
BLAH;
    ...
    function genInputStandard(&$aInput) {
		...
		/* Deano Datapicker Fix - Start */
		if($aInput['type'] == 'date') {
			$aAttrs['class'] = $aAttrs['class'] . ' deanodate'; 
			$aAttrs['type'] = 'text';
			$this->hasDatePicker = true;
		}
		if($aInput['type'] == 'datetime') {
			$aAttrs['class'] = $aAttrs['class'] . ' deanodatetime'; 
			$aAttrs['type'] = 'text';
			$this->hasDatePicker = true;
		}
		/* Deano Datapicker Fix - End */
