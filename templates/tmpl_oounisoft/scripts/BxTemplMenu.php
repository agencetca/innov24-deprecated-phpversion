...
EOF;
	...
	*/
	function getCode() {
		//added by AFK -- Menu control for plugin configure me
        /*  URL structure
         *  blog/news : ... /blogs/category/Dating+and+Personals
         *  Photo : ... /m/photos/browse/category/catname
         *  Video : ... /m/videos/browse/category/catname
         *  Event : ... /m/events/browse/category/catname
         *  file : ... /m/files/browse/category/catname
         *  group : ... /m/groups/browse/category/catname
         *  store : ... /m/store/browse/category/catname
         *  gigs : ... /m/gigs/browse/category/catname
         *  opinions : ... /m/opinions/browse/category/catname
         *  pages : ... /m/pages/browse/category/catname
         */
        $structure = array( // get specified url cat | name need to be same than afk_cfg_me module
            'Events'	=> array(
            	'base_url' => "m/events/",
            	'url' => "browse/category/",
            	'id' => "11000",
...
//             	),
        ...
        $cfgmeMenu = array(
                ...
                'Check'     => "",
                'Parent'    => "0", //0 defaut
                'Picture'   => "modules/boonex/blogs/|bx_blogs.png",
                'Icon'      => "",
                'BQuickLink'=> "0",
                'Statistics'=> "",
                'Name'      => "Test"
        ...
        $controlCfgme=@mysql_query("SELECT cat_id FROM afk_cfgme_interest WHERE user_id='$_COOKIE[memberID]'");
        if($catCfgmeID=@mysql_result($controlCfgme, 0)){ //if result configured
            $asinterest=explode('{', $catCfgmeID);
            $catCfgmeID=explode('}', $catCfgmeID);
            $asinterest=array_slice($asinterest, 0, -1); //remove last row over generated
            if(sizeof($asinterest)<1) // check if oat least one cat as selected
            	$this->aTopMenu['900']=$cfgmeMenu;
            $catCfgmeID=array_slice($catCfgmeID, 0, -1); //remove last row over generated
            foreach ($catCfgmeID as $key => $value) {
                $tempInt=explode('{', $catCfgmeID[$key]); //$tempInt[0]= Events | $tempInt[1] = Expedition,birthday,etc.
                if(!empty($tempInt[1])){    // filter empty cat
                    $this->aTopMenu[$structure[$tempInt[0]]['id']]=array(
                        'Type'      => "top",
		                'Caption'   => _t($tempInt[0]),
		                'Link'      => $structure[$tempInt[0]]['base_url'],
		                'Visible'   => "memb",
		                'Target'    => "",
		                'Onclick'   => "",
		                'Check'     => "",
                    ...
                    $tempExplode=explode(',',$tempInt[1]);
                   //  foreach ($tempExplode as $sub_category_name) { //init subcat
//                     	// $sub_category_name=str_replace(' ', '+', $sub_category_name);
// 	                    $this->aTopMenu[]=array(
// 	                        'Type'      => "custom",
// 			                'Caption'   => $sub_category_name,
// 			                'Link'      => $structure[$tempInt[0]]['base_url'].$structure[$tempInt[0]]['url'].$sub_category_name,
// 			                'Visible'   => "memb",
// 			                'Target'    => "",
// 			                'Onclick'   => "",
// 			                'Check'     => "",
// 			                'Parent'    => $structure[$tempInt[0]]['id'], // get id specifier
// 			                'Picture'   => "",
// 			                'Icon'      => "",
// 			                'BQuickLink'=> "0",
// 			                'Statistics'=> "",
...
// 	                    }
			...
			$this->aTopMenu['900']=$cfgmeMenu;//add cfgme menu
	        // var_dump($this->aTopMenu);
	        // $cfgmeMenu = array( //sub itm exemple
	        //         'Type'      => "top",
	        //         'Caption'   => "sub cfg Me",
	        //         'Link'      => "m/configure/",
	        //         'Visible'   => "memb",
	...
	*/
	function genMoreLanguagesElement(){
		...
		foreach ($aExistedLanguages as $sLanguageID => $sLanguageVal) {
			...
			$sLanguageElement .= '<li><a href="' . $sLanguageUrl . '" value="' . $sLanguageVal . '" class="button more_ntop_element' . $sIActiveClass . '"><img class="languageflag" src="/media/images/flags/'.$sLanguageID.'.gif">'.$sLanguageVal.'</a>';
		}
...
EOF;
	...
	function genBreadcrumb($aPath = array()) {
		...
		$sDivider = '<img class="bc_divider" src="/templates/tmpl_oounisoft/images/bc_divider.png" />';
		$aPathLinks = array();
