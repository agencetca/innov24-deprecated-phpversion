	...
	*/
	...
	//--- AQB Profile Types Splitter ---//
		...
		if ($aCoupleInfo) {
			...
			$sProfile2Match = ($bExtMode && isLogged() && ( $iVisitorID != $aCoupleInfo['ID'] ) && getParam('view_match_percent') && getParam('enable_match')) ? $GLOBALS['oFunctions']->getProfileMatch( $iVisitorID, $aCoupleInfo['ID'] ) : '';
		} else {
			$sProfile2ASc2 = 'display:none;';
		}

		// match progress bar
		$sProfileMatch = ($bExtMode && isLogged() && ( $iVisitorID != $aProfileInfo['ID'] ) && getParam('view_match_percent') && getParam('enable_match')) ? $GLOBALS['oFunctions']->getProfileMatch( $iVisitorID, $aProfileInfo['ID'] ) : '';
