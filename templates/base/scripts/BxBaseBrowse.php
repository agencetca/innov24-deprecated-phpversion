	...
	function getBlockCode_SearchedMembersBlock() {
		...
		while( true == ($aRow = mysql_fetch_assoc($rResult)) ) {
			...
			if ( $aRow['Couple']) {        
				...
				if ( !($iIndex % 2)  ) {
					$sOutputHtml .= $this -> oSearchProfileTmpl -> PrintSearhResult($aRow, $aCoupleInfo, null, $sTemplateName);
				} else {
					// generate the filled block ;
					$sOutputHtml .= $this -> oSearchProfileTmpl -> PrintSearhResult($aRow, $aCoupleInfo, $aExtendedCss, $sTemplateName);
				}
			} else { // generate the `single` thumbnail ;
				if ( !($iIndex % 2)  ) {
					$sOutputHtml .= $this -> oSearchProfileTmpl -> PrintSearhResult($aRow, '', null, $sTemplateName);
				} else {
					// generate the filled block ;
					$sOutputHtml .= $this -> oSearchProfileTmpl -> PrintSearhResult($aRow, '', $aExtendedCss, $sTemplateName);
				}
			}
			$iIndex++;
		}
		...
		(
			...
			(
				...
				'per_page_step'      => ( $this -> aDisplaySettings['mode'] == 'extended' ) ? 5 : 16,
			)
		);
		...
		return array(  $sOutputHtml, $aToggleEllements );
	}
}
