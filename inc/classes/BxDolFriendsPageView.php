    ...
    class BxDolFriendsPageView extends BxDolPageView 
    {
    	...
    	*/
    	function BxDolFriendsPageView($sPageName, &$aDisplayParameters, $iProfileID)
    	{
    		...
    		$this -> sCurrentPage = $_SERVER['PHP_SELF'];	
            ...
            if ( $iMemberId ) {
                ...
                "
            		...
            		FROM 
                        `Profiles` AS p
            		LEFT JOIN 
                        `sys_friend_list` AS f1 
                    ON 
                        (
                            f1.`ID` = p.`ID` AND f1.`Profile` ={$iMemberId}
                                AND 
                            `f1`.`Check` = 1
                        )
            		LEFT JOIN 
                        `sys_friend_list` AS f2 
                    ON 
                        (
                            f2.`Profile` = p.`ID` 
                                AND 
                    ...
                    (
                        ...
                        AND 
                    (
                        p.`DateLastNav` > SUBDATE(NOW(), INTERVAL " . $iOnlineTime . " MINUTE)
                    )
                ";
               ...
               // $aFriends = array_reverse($aFriends);
                ...
                if ( $iOnlineFriends > $iOldCount) {
                    ...
                    for( $i = $iOldCount; $i < $iOnlineFriends; $i++)
                    {
                        ...
                        $aKeys = array (
                            'sender_thumb'    => $oFunctions -> getMemberThumbnail($aFriends[$i][0], 'left'),
