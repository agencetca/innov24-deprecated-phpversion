...
if( $this -> iAreaID == 1 ) { // join
	...
	if ( isset( $_POST['v_field'] ) ) {
		$aFields = array();
		
		$aVField = explode( '[', $_POST['v_field'] );
		$_POST['v_field'] = $aVField[0];
		foreach ( $this->aBlocks as $iBlockID => $aBlock ) {
			foreach ( $aBlock['Items'] as $iFieldID => $aField ) {
				if ( $aField['Name'] == $_POST['v_field'] || ( str_replace( $aField['Name'], '', $_POST['v_field'] ) == '_confirm' ) ) {
					if ( $aField['Type'] == 'pass' && !strstr( $_POST['v_field'], '_confirm' ) && !empty( $_POST[$_POST['v_field']][0] ) )
						break;
					$aFields[$iBlockID] = array(
						'Caption'	=> $aBlock['Caption'],
						'Items'		=> array( $iFieldID => $aField ),
					);
					break;
				}
			}
		}
		$this->aBlocks = $aFields;
	}
}
/*---------------------------------- join_validation ----------------------------------[End]*/
...
//--- AQB Dependent Fields ---//
return $sDFCache . $sCustomHtmlBefore . $oForm->getCode() . $sCustomHtmlAfter;
//--- AQB Dependent Fields ---//
    }
...
//--- AQB Dependent Fields ---//
    ...
    function convertEditField2Input($aItem, $aParams, $iPerson) {
        ...
        switch ($aItem['Type']) {
            ...
            case 'html_area':  $aInput['type'] = 'textarea'; $aInput['html'] = true; $aInput['value'] = $aValues[$iPerson]; $aInput['attrs']['counter'] = 'true'; break;
