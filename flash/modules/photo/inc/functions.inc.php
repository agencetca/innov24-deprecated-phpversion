...
function photo_getEmbedThumbnail($sUserId, $sImageUrl)
{
	...
	$sCommand = $sFfmpegPath . " -y -i " . $sImageUrl . " -ss 0 -vframes 1 -an -sameq -f image2 " . $sFilePath;
	@popen($sCommand, "r");
...
function photo_getRecordThumbnail($sUserId)
{
	...
	$sCommand = $sFfmpegPath . " -y -i " . $sFilesPath . $sUserId . IMAGE_EXTENSION . " -s 64x64 -ss 0 -vframes 1 -an -sameq -f image2 " . $sFilesPath . $sFileName;
	@popen($sCommand, "r");
