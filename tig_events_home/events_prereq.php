...
//====================================================================================================================//
				...
				$LienProfil = getProfileLink($contenu['ResponsibleID']);
				...
				$LienPost = "m/events/view/".$contenu['EntryUri']."";

				//On prépare l'affichage des sur-catégories (les 'Types' de fawn)
				$SurCategorie = _t('_bx_event_Events');
				...
				$snip = (strlen($contenu['Description']) > 500) ? substr($contenu['Description'], 0, 500) . '...' : $contenu['Description'];
				...
				$Date = '<p class="datestyle">'._t('_wipM_creation_date').' : '.DataTime($contenu['Date'],$dateFormatC).'</p>';
				...
				if ($contenu['PrimPhoto']!=NULL){
					$PrimPhoto = '<div class="blog_photos"><img src="media/images/blog/big_'.$contenu['PrimPhoto'].'" width="auto" height="64"/></div>';
				}else{$PrimPhoto = '';}

				//On défini la privacy du post et l'image qui doit s'afficher en conséquence
				if ($contenu['allow_view_event_to'] == 3){
				$PrivacyDiv = '<div class="privacypic4"></div>';
				}
				else if ($contenu['allow_view_event_to'] == 4){
				$PrivacyDiv = '<div class="privacypic1"></div>';
				}
				else if ($contenu['allow_view_event_to'] == 5){
