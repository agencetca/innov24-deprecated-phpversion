...
class PommoHelperMessages {
	...
	function testExchanger($to,$exchanger) {
		...
		$subject = Pommo::_T('test message');
		$body = sprintf(Pommo::_T("This message indicates that it is able to use the %s exchanger."),$exchanger);
	...
	function notify(&$notices,&$sub,$type,$comments=false) {
		...
		$body = sprintf(Pommo::_T('Notice'),$type);
