...
EOF;
	...
	function AddNewPostForm($iPostID=0, $bBox = true) {
		...
		$this->CheckLogged();
		$result55 = mysql_query("SELECT * FROM bx_ads_main WHERE ID = '$iPostID' ");
	...
	$oAlert = new BxDolAlerts('ads', 'pre_create', $this->_iVisitorID);
	$oAlert->alert();  
...
EOF;
        ...
        asort($aCountries);
		$aTypes = $oProfileFields->convertValues4Input('#!AdsType');
        asort($aTypes);
		...
		$aForm = array(
			...
			'inputs' => array(
				...
				'adstype' => array(
                    'type' => 'select',
                    'name' => 'adstype',
                    'caption' => _t('_bx_ads_form_caption_type'),
                    'values' => $aTypes,
                    'required' => true,
                    'checker' => array (
                        'func' => 'preg',
                        'params' => array('/^[A-Za-z0-9]{2,4}$/'),
                        'error' => _t ('_bx_ads_err_type'),
                    	),                                                
                    'db' => array (
                        'pass' => 'Preg', 
                        'params' => array('/([A-Za-z0-9]{2,4})/'),//Regular expression permet A à Z et 0 à 9 longueur entre 3 et 4 caractères ^^
                    ),
                    'display' => true,
                ),
				'LifeTime' => array(
