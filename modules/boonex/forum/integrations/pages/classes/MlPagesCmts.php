...
class MlPagesCmts extends BxTemplCmtsView {
    
	/**
	 * Constructor
	 */
    function MlPagesCmts($sSystem, $iId) {
    ...
    function isPostReplyAllowed () {
        ...
        return $oMain->isAllowedComments($aPage);
    }

    function isEditAllowedAll () {       
        $oMain = $this->getMain();
        $aPage = $oMain->_oDb->getEntryByIdAndOwner($this->getId (), 0, true);
        if ($oMain->isAllowedCreatorCommentsDeleteAndEdit ($aPage))
            return true;
