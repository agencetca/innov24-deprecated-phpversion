...
{	
	...
	{				
		...
		switch ($type)
		{
			case 'msgs':
			case 'tlts':
				$a = $this->fdb->searchMessages (filter_to_db($text), filter_to_db($u), $forum, $type, ('posts' == $disp ? 1 : 0), $max_res);
				break;
			default:
...
EOF;
		...
		return "<root>$cu<new_topic sticky=\"$sticky\">{$x2}{$x1}<signature>{$signature}</signature></new_topic></root>";
	}
...
EOF;
	...
	function getHiddenPostXML ($post_id, $force_show)
	{
		...
		{				
			...
			$ui[$r['user']] = array ('posts' => (int)$this->fdb->getUserPosts($r['user']), 'avatar' => $aa['avatar'], 'url' => $aa['profile_url'], 'onclick' => $aa['profile_onclick'], 'role' => $aa['role']);
		}
	...
	function getPostsXML ($topic_uri, $wp)
	{
		...
		while ( list (,$r) = each ($a) )
		{
			...
			{				
				...
				$ui[$r['user']] = array ('posts' => $this->fdb->getUserPosts($r['user']), 'avatar' => $aa['avatar'], 'url' => $aa['profile_url'], 'onclick' => $aa['profile_onclick'], 'role' => $aa['role'], 'signature' => $signature);
			}
...
EOF;
	 ...
	 * @param $wp			return whole page XML
	 */
	function getXxxTopicsXML ($wp, $sTitle, $sDesc, $sFunc)
	{
		global $gConf;
...
EOF;
		...
		if ($wp)
		{
			...
			return $this->addHeaderFooter ($li, "<topics>{$x2}{$x1}{$t}</topics>");
		}
		else
		{
			$cu = $this->getUrlsXml ();
			return "<root>$cu<topics>{$x2}{$x1}{$t}</topics></root>";
		}
	}
	 ...
	 * @param $wp			return whole page XML
	 */
	function getHiddenTopicsXML ($wp)
	{
        return $this->getXxxTopicsXML ($wp, 'Hidden Topics', '_Hidden topics', 'getHiddenTopics');
	}
	...
	{
        return $this->getXxxTopicsXML ($wp, 'My Topics', 'Topics you participate in', 'getMyThreadsTopics');
	}
	...
	{
        return $this->getXxxTopicsXML ($wp, 'Topics subscriptions', 'Topics you have subscribed for', 'getMyFlaggedTopics');
	}
	...
	function getLivePostsXML ($count = 10, $ts = 0)
    {
		...
		while (list(,$r) = each ($a))
        {
            ...
            if (!isset($ui[$r['user']]))
            {
                $aa = $this->_getUserInfo ($r['user']);
                encode_post_text ($aa['role']);
                $ui[$r['user']] = array ('avatar' => $aa['avatar'], 'url' => $aa['profile_url'], 'onclick' => $aa['profile_onclick'], 'role' => $aa['role']);
            }
...
EOF;
		...
		$iReplyId = $this->fdb->postReply ($p['forum_id'], $p['topic_id'], $p['topic_text'], $user);
        ...
        $t = $this->fdb->getTopic($p['topic_id']);
...
EOS;
	...
	function postNewTopicXML ($p)
	{
		...
		if (!$this->_checkUserPerm ('', $f['forum_type'], 'post', (int)$p['forum_id']) || bx_is_spam($p['topic_text'])) 
		{
			return <<<EOF
<html>
<body>
...
EOF;
		...
		$post_id = $this->fdb->newTopic ((int)$p['forum_id'], $p['topic_subject'], $p['topic_text'], ($p['topic_sticky'] == 'on'), $user, $topic_uri);
        ...
        if (is_callable($this->onNewTopic))
            call_user_func_array ($this->onNewTopic,  array((int)$p['forum_id'], $p['topic_subject'], $p['topic_text'], ($p['topic_sticky'] == 'on'), $user, $topic_uri));
...
EOF;
	...
	function getRssUpdatedTopics ()
	{
		...
		while ( list (,$r) = each ($a) )
		{
			$td = orca_mb_replace('/#/', $r['count_posts'], '# posts') . ' &#8226; ' . orca_mb_replace('/#/', $r['last_post_user'], 'last reply by #') . ' &#8226; ' . $r['cat_name'] . ' &#187; ' . $r['forum_title'];
	...
	function getRssTopic ($topic_uri)
	{
		...
		$a = $this->fdb->getPosts ($topic_id, 0);
		...
		while ( list (,$r) = each ($a) )
		{
			$lp = $this->fdb->getTopicPost((int)$r['topic_id'], 'last');
            $td = strip_tags(orca_mb_substr($r['post_text'], 0, 255));
            if (orca_mb_len($td) == 255) $td .= '[...]';
			$tt = orca_mb_substr($td, 0, 32);
			
				$lastBuildDate = $lp['when'];
        
            $items .= $this->_rssItem ($tt, sprintf($gConf['rewrite']['topic'], $t['topic_uri']), $td, $lp['when']);
		}		
	...
	function getRssUser ($user, $sort)
	{
        ...
        $sTitle = sprintf("%s's forum posts", $user);

        return $this->_rssFeed ($sTitle, '', $sTitle, $lastBuildDate, $items);
    }
    ...
    function _rssPrepareConf () {
		...
		$gConf['date_format'] = '%a, %e %b %Y %k:%i:%s';
    }

    function _rssItem ($sTitle, $sLink, $sDesc, $sDate) {
        global $gConf;
		return "
			<item>
				<title><![CDATA[{$sTitle}]]></title>
    ...
    function _rssFeed ($sTitle, $sLink, $sDesc, $sLastDate, $sItems) {
        global $gConf;
		return <<<EOF
<rss version="2.0">
	<channel>
...
EOF;
	...
	function showProfile ($u, $wp)
	{
		...
		$a['last_online'] = $this->fdb->getUserLastOnlineTime (filter_to_db($u));
