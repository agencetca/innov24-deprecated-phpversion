...
class BxVideosPageAlbumsMy extends BxDolPageView {
    ...
    function getBlockCode_add () {
        ...
        if ($oForm->isSubmittedAndValid()) {
            ...
            if ($iNewId > 0) {
                ...
                $sUrlAdd = BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() . 'albums/my/add_objects/' . $aNew['Uri'] . '/owner/' . getNickName($this->iOwnerId);
	...
	$oAlert = new BxDolAlerts($this->oConfig->getMainPrefix(), 'pre_add', $this->iOwnerId);
	$oAlert->alert();  
	//[end] credits mod (modzzz.com)
            ...
            $sCode = '<div class="bx_sys_default_padding">' . $oUploader->GenMainAddVideoForm(array('album' => $this->aAddParams[1])) . '</div>';
        }
    ...
    function getBlockCode_delete () {
        ...
        $aForm = array(
            ...
            'inputs' => array(
                'info' => array(
                    'type' => 'custom',
                    'content' => _t('_' . $this->oConfig->getMainPrefix() . '_album_delete_warning')
                ),
                'submit' => array(
                    'type' => 'submit',
                    'name' => 'submit',
                    'value' => _t('_' . $this->oConfig->getMainPrefix() . '_album_delete_confirm'),
                    'attrs' => array(
    ...
    function getAdminObjectPart ($aCondition = array(), $bAdminMode = true, $iBoxId = 0, $bShowMove = false) {
        ...
        $aUnit = array(
            'main_code' => $sCode . $sPaginate . $sManage,
            'bx_if:hidden' => array(
                'condition' => (int)$aAlbumInfo['ID'] != 0,
                'content' => array(
