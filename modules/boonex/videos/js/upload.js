...
BxVideoUpload.prototype.changeContinueButtonStatus = function () {
	switch(this.getType()) {
		case 'upload':
			...
			var sAgreeVal = $('#video_upload_form #video_upload_form_input_agree').attr('checked');
...
BxVideoUpload.prototype.doValidateFileInfo = function(oButtonDom, iFileID) {
	...
	if ($('#send_file_info_' + iFileID + ' textarea[name=description]').val()=='') {
		$('#send_file_info_' + iFileID + ' textarea[name=description]').parent().parent().parent().children('.warn').show().attr('float_info', _t('_bx_videos_val_descr_err'));
		bRes = false;
	}
	else
...
BxVideoUpload.prototype.checkEmbed = function () {
	var tText = $('#video_upload_form [name="embed"]');
	...
	var bResult = (/^http:\/\/(www.)?youtube.com\/watch\?v=([0-9A-Za-z_-]{11})$/.test(sText) || /^http:\/\/(www.)?youtu.be\/([0-9A-Za-z_-]{11})$/.test(sText)) && $('#accepted_files_block').text() == "";
	if(bResult)
		tText.attr("value", sText);
	shVideoEnableSubmit(bResult);
}