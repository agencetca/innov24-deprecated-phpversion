...
class BxPhotosPageAlbumsMy extends BxDolPageView {
    ...
    function BxPhotosPageAlbumsMy (&$oShared, $iOwnerId, $aParams = array()) {
        ...
        $this->oSearch = new BxPhotosSearch('album', $this->aAddParams[1], 'owner', getNickName($this->iOwnerId));
	...
	//[end] credits mod (modzzz.com)
            ...
            $sCode = '<div class="bx_sys_default_padding">' . $oUploader->GenMainAddPhotosForm(array('album' => $this->aAddParams[1])) . '</div>';
        }
    ...
    function getAdminPart ($aCondition = array(), $aCustom = array()) {
        ...
        $iPerPage = (int)$this->oConfig->getGlParam('number_albums_home');
        $iPage = isset($_GET['page']) ? (int)$_GET['page'] : $this->oSearch->aCurrent['paginate']['page'];
