...
EOF;
		...
		if ($this->iPostViewType==4) {
			...
			$sPostPicture2 = <<<EOF
	<div class="thumbnail_block" style="float:left;width:38px;height:38px;">
	<div class="thumbnail_image" style="width:36px;height:36px;">
	<a href="{$sPostLink}"><img src="{$sBlogsImagesUrl}small_{$aResSQL['PostPhoto']}" alt="{$aResSQL['PostPhoto']}" width="32" height="32" /></a>
	</div></div>
EOF;
}else{
	$sPostPicture2 = $GLOBALS['oFunctions']->getMemberIcon($aResSQL['ownerId'], 'left');
}
		}
		...
		$aUnitReplace['post_picture3'] = $sPostPicture2;
