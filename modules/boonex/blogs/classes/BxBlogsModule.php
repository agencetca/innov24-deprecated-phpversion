...
EOF;
    ...
    function getBlogPostBlock() {
		...
		$sPostCaption = _t(_Description);
		
		$sql_type = "SELECT * FROM sys_pre_values,bx_blogs_posts WHERE sys_pre_values.Value = bx_blogs_posts.NewsType";
		$req_type = mysql_query($sql_type);
		while ($result_type = mysql_fetch_assoc($req_type)){
		$type ='<h1><font color="#FFBEA3">'._t($result_type['LKey']).'</font></h1>';
		}
        $sPostString = $sBlogPostsVal;
		//var_dump($sPostString);
		$sPostStringVal = <<<EOF
<div class="blogs-view">
	{$type}
	{$sPostString}
...
EOF;
	...
	function GenMyPageAdmin($sMode = '') {
		...
		$sMainTabClass = $sAddTabClass = $sManageTabClass =0; /* $sPendingTabClass = 0; */
		switch($sMode) {
			case 'add':
				$sAddTabClass = 1;
...
EOF;
	...
	function AddNewPostForm($iPostID=0, $bBox = true) {
		...
		$result55 = mysql_query("SELECT * FROM bx_blogs_posts WHERE PostID = '$iPostID' ");
        ...
        asort($aCountries);
		$aTypes = $oProfileFields->convertValues4Input('#!NewsType');
        asort($aTypes);
		...
		$aForm = array(
			...
			'inputs' => array(
				...
				'NewsType' => array(
                    'type' => 'select',
                    'name' => 'NewsType',
                    'caption' => _t('_bx_news_form_caption_type'),
                    'values' => $aTypes,
                    'required' => true,
                    'checker' => array (
                        'func' => 'preg',
                        'params' => array('/^[A-Za-z0-9]{3,6}$/'),
                        'error' => _t ('_bx_news_err_type'),
                    	),                                
                    'db' => array (
                        'pass' => 'Preg', 
                        'params' => array('/([A-Za-z0-9]{3,6})/'),//Regular expression permet A à Z et 0 à 9 longueur entre 3 et 4 caractères ^^
                    ),
                    'display' => true,
                ),
				'PostText' => array(
				...
				'Country' => array(
                    'type' => 'select',
                    'name' => 'Country',
                    'caption' => _t('_bx_news_caption_country'),
                    'values' => $aCountries,
                    'required' => true,
                    'checker' => array (
                        'func' => 'preg',
                        'params' => array('/^[a-zA-Z]{2}$/'),
                        'error' => _t ('_bx_news_err_country'),
                    ),                                        
                    'db' => array (
                        'pass' => 'Preg', 
                        'params' => array('/([a-zA-Z]{2})/'),
                ...
                'City' => array(
                    'type' => 'text',
                    'name' => 'City',
                    'caption' => _t('_bx_news_caption_city'),
                    'required' => true,
                    'checker' => array (
                        'func' => 'length',
                        'params' => array(3,50),
                        'error' => _t ('_bx_news_err_city'),
                    ),
                    'db' => array (
                        'pass' => 'Xss', 
                    ),                
				...
				'Active_New' => array(
					...
					'caption' => _t('_bx_news_caption_open'),
...
EOF;
	...
	*/ 
	function serviceBlogsIndexPage() {
		...
		$oBlogSearch->aCurrent['restriction']['allow_view']['value']  = BX_DOL_PG_ALL; 
