...
// setting the emmetbytes avatar
    ...
    function serviceGetSiteAvatars ($iPage) {
        ...
        $sScript = "<script>
            $(document).ready(function() {
                $('#bx_ava_site_avatars .bx_ava_tar .bx_ava_actions a img').bind('click', function (e) {
                    var eImg = $(this);
                    getHtmlData('bx_ava_my_avatars', '" . $this->_oConfig->getBaseUri() . "set_site_avatar/' + $(this).attr('alt'), function () {
                        eImg.parent().parent().html('"._t('_bx_ava_ok')."');
                    }, 'post');
                });
            });
        </script>";
        ...
        return $sAjaxPaginate . $GLOBALS['oFunctions']->centerContent($this->_oTemplate->parseHtmlByName ('avatars_site', $aVars), '.bx_ava_tar') . $sAjaxPaginate . $sScript;
    }
    ...
    function serviceManageAvatars ($iProfileId, $isAddJsCss = false) {
        ...
        return $this->_oTemplate->parseHtmlByName ('manage_avatars', $aVars);
    }
    ...
    function serviceGetMyAvatars ($iProfileId) {
        ...
        $sScript = "<script>
            $(document).ready(function() {
                $('#bx_ava_my_avatars .bx_ava_tar .bx_ava_actions a.bx_ava_set_couple img').bind('click', function (e) {
                    getHtmlData('bx_ava_my_avatars', '" . $this->_oConfig->getBaseUri() . "set_avatar_couple/$iProfileId/' + $(this).attr('alt'), false, 'post');
                });
                $('#bx_ava_my_avatars .bx_ava_tar .bx_ava_actions a.bx_ava_set img').bind('click', function (e) {
                    getHtmlData('bx_ava_my_avatars', '" . $this->_oConfig->getBaseUri() . "set_avatar/$iProfileId/' + $(this).attr('alt'), false, 'post');
                });
                $('#bx_ava_my_avatars .bx_ava_tar .bx_ava_actions a.bx_ava_del img').bind('click', function (e) {
    ...
    function serviceGetWallPost ($aEvent) {
        ...
        $aVars = array(
                ...
                'cpt_added_new' => _t('_bx_ava_wall_added_new'),
                'cpt_object' => _t('_bx_ava_wall_object'),
                'post_id' => $aEvent['id'],
        );
        return array(
            'title' => $aProfile['username'] . ' ' . _t('_bx_ava_wall_added_new') . ' ' . _t('_bx_ava_wall_object'),
	...
	function serviceGetWallData () {
	    return array(
            'handlers' => array(
                array('alert_unit' => 'bx_avatar', 'alert_action' => 'add', 'module_uri' => 'avatar', 'module_class' => 'Module', 'module_method' => 'get_wall_post')
            ),
            'alerts' => array(
