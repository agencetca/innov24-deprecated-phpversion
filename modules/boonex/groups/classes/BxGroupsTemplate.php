...
class BxGroupsTemplate extends BxDolTwigTemplate {
    ...
    function unit ($aData, $sTemplateName, &$oVotingView) {
        ...
        $aVars = array (            
			...
			'type' =>_t( $GLOBALS['aPreValues']['GroupType'][$aData['type']]['LKey']),
 ...
 function blockDesc (&$aDataEntry) {
	$fin = strlen($aDataEntry['title']);
	$coupe1 = 57;
	$coupe2= 114;
	$titre_part1=substr($aDataEntry['title'],0,$coupe1);
	$titre_part2=substr($aDataEntry['title'],$coupe1,$coupe1);
	$titre_part3=substr($aDataEntry['title'],$coupe2,$coupe1);
	//echo strlen($aDataEntry['title']);  Voir la longueur de la chaine de description
		if ($fin>57){
		$aVars = array (
		'description' => '<h1><font color="#FFBEA3">'._t($GLOBALS['aPreValues']['GroupType'][$aDataEntry['type']]['LKey']).'</font></h1><h2><font color="#FF0000">'.$titre_part1.'<br>'.$titre_part2.'<br>'.$titre_part3.'</font></h3><p>'.$aDataEntry['desc'].'',);
		return $this->parseHtmlByName('block_description', $aVars);
    }
		else{
		$aVars = array (
		'description' => '<h1><font color="#FFBEA3">'._t($GLOBALS['aPreValues']['GroupType'][$aDataEntry['type']]['LKey']).'</font></h1><h2><font color="#FF0000">'.$aDataEntry['title'].'</font></h3><p>'.$aDataEntry['desc'].'',);
		return $this->parseHtmlByName('block_description', $aVars);
    }}
    ...
    function blockFields (&$aDataEntry) {
        ...
        foreach ($oForm->aInputs as $k => $a) {
			...
			//On affiche la langage key
                ...
                $sRet .= _t($GLOBALS['aPreValues']['GroupType'][$aDataEntry[$k]]['LKey']);	
            else
                $sRet .= $aDataEntry[$k];
            $sRet .= '<td></tr>';
        }
