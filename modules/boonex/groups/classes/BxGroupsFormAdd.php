...
class BxGroupsFormAdd extends BxDolFormMedia {
    ...
    function BxGroupsFormAdd ($oMain, $iProfileId, $iEntryId = 0, $iThumb = 0) {
        ...
        $result55 = mysql_query("SELECT * FROM bx_groups_main WHERE ID = '$iEntryId' ");
        ...
        asort($aCountries);
		$aTypes = $oProfileFields->convertValues4Input('#!GroupType');
        asort($aTypes);
        ...
        $aCustomForm = array(
            ...
            'inputs' => array(
				...
				'type' => array(
                    'type' => 'select',
                    'name' => 'type',
                    'caption' => _t('_bx_groups_form_caption_type'),
                    'values' => $aTypes,
                    'required' => true,
                    'checker' => array (
                        'func' => 'preg',
                        'params' => array('/^[A-Za-z0-9]{3,6}$/'),
                        'error' => _t ('_bx_events_err_type'),
                    	),                                         
                    'db' => array (
                        'pass' => 'Preg', 
                        'params' => array('/([A-Za-z0-9]{3,4})/'),//Regular expression permet A à Z et 0 à 9 longueur entre 3 et 4 caractères ^^
                    ),
                    'display' => true,
                ),
                'title' => array(
				...
				'Deadline' => array(
                    'type' => 'datetime',
                    'name' => 'Deadline',
                    'caption' => _t('_bx_groups_caption_deadline'),
                    'required' => true,
                    'checker' => array (
                        'func' => 'DateTime',
                        'error' => _t ('_bx_groups_err_deadline'),
                    ),
                    'db' => array (
                        'pass' => 'DateTime', 
                    ),    
                ...
                'Active_Group' => array(
					...
					'caption' => _t('_bx_groups_caption_open'),
