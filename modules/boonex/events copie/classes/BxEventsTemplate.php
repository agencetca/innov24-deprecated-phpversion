...
class BxEventsTemplate extends BxDolTwigTemplate {
    ...
    function unit ($aData, $sTemplateName, &$oVotingView) {
        ...
        $aVars = array (            
			...
			//On construit l'objet à afficher
	...
	$titre_part2=substr($aEvent['Title'],$coupe1,$fin);
	//$titre_part3=substr($aEvent['Title'],$coupe2,$fin);
	echo strlen($aEvent['Title']);
		if ($fin>57){
		$aVars = array (
		'description' => '<h1><font color="#FFBEA3">'._t($GLOBALS['aPreValues']['EventType'][$aEvent['Type']]['LKey']).'</font></h1><h2><font color="#FF0000">'.$titre_part1.'<br>'.$titre_part2.'</font></h3><p>'.$aEvent['Description'].'',);
