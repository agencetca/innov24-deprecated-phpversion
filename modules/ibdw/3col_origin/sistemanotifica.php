...
$aParams = serialize($array);
  ...
  if ($scelta=="ok") 
  { 
   $querydecidi="UPDATE sys_friend_list SET sys_friend_list.Check = 1 WHERE ID =". $richiesta . " AND Profile=" . $ottieniid;
   $resultdecidi = mysql_query($querydecidi) or die(mysql_error());
   $inseriscispy = "INSERT INTO `bx_spy_data` (sender_id,recipient_id,lang_key,params,type) VALUES ('$ottieniid','$richiesta','_bx_spy_profile_friend_accept','$aParams','profiles_activity')";
   $esecuzione = mysql_query($inseriscispy);
  }
  elseif ($scelta=="no") 
  { 
   $querydecidi="DELETE FROM `sys_friend_list` WHERE Profile=" . $ottieniid . " AND ID=" . $richiesta ;
   $resultdecidi = mysql_query($querydecidi) or die(mysql_error());
  }
?>