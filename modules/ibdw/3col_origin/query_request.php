...
//nuovi saluti
$query = "SELECT SUM(Number) FROM `sys_greetings` WHERE (`Profile`=" . $ottieniID . " AND `New`='1')";
...
$query = "SELECT ID FROM `sys_friend_list` WHERE `Profile`=" . $ottieniID . " AND `Check`=0";
...
if ($showfriendsreq=="ON")
{
 if ($contareqamicizie>0) 
 {
  ...
  for ($numami=0;$numami<$contareqamicizie;$numami++)
  {
   ...
   if (!is_friends($richiestaamiciaz[0],$ottieniID))
   {
   ...
   {
	if (CountMutualFriends($ottieniID,$amichetto['ID'])>0) {echo '<div id="amicomuni">'._t('_ibdw_thirdcolumn_mutualfriends').CountMutualFriends($ottieniID,$amichetto['ID']).'</div>';}
    else {echo '<div id="distanziatore"></div>';}
   }                 
   echo '<div class="pulsante"><form id="pulsantedscelta'.$richiestaamiciaz[0].'" action="javascript:elimina();"><input id="richiestaamiciaz'.$richiestaamiciaz[0].'"  type="hidden" name="id" value="'.$richiestaamiciaz[0].'"><input id="3colid'.$richiestaamiciaz[0].'"  type="hidden" name="id" value="'.$ottieniID.'"><input id="scelta'.$richiestaamiciaz[0].'"  type="hidden" name="id" value="ok"><input type="submit" value="'._t('_ibdw_thirdcolumn_accept').'" class="click"> </form></div><div class="pulsante"> <form id="pulsantedsceltano'.$richiestaamiciaz[0].'" action="javascript:elimina();"><input id="nrichiestaamiciaz'.$richiestaamiciaz[0].'"  type="hidden" name="id" value="'.$richiestaamiciaz[0].'"><input id="nid'.$richiestaamiciaz[0].'"  type="hidden" name="id" value="'.$ottieniID.'"><input id="nscelta'.$richiestaamiciaz[0].'"  type="hidden" name="id" value="no"><input type="submit" value="'._t('_ibdw_thirdcolumn_reject').'" class="click"></form></div></div></div></div>
           ...
           $.ajax({
 			     type: "POST",
 			     url: "modules/ibdw/3col/sistemanotifica.php",
 			     data: "ottieniid=" + ottieniid + "&richiestaamiciaz=" + richiesta +"&scelta=" + scelta + "&nameformat=" + '.$usernamef.',
           ...
           $.ajax({
 			     type: "POST",
 			     url: "modules/ibdw/3col/sistemanotifica.php",
 			     data: "ottieniid=" + ottieniid + "&richiestaamiciaz=" + richiesta +"&scelta=" + scelta,
 			     success: function(html){
                       $("#richiesta'.$richiestaamiciaz[0].'").fadeOut();
                       aggiornajax();
                       }
  ...
  {
   $query = "DELETE FROM `sys_friend_list` WHERE `Profile`=" . $ottieniID . " AND ID=".$richiestaamiciaz[0]." AND `Check`=0";
   $resultreqfriends = mysql_query($query);
  }
  }
  echo '</div>';
...
$query = "(SELECT ID FROM sys_friend_list WHERE Profile=".$ottieniID." AND sys_friend_list.Check=1) UNION (SELECT Profile FROM sys_friend_list WHERE ID=".$ottieniID." AND sys_friend_list.Check=1)";
...
//ottengo gli id degli utenti che ho bloccato
 $getidblocked="SELECT sys_block_list.Profile FROM sys_block_list WHERE sys_block_list.ID=".$ottieniID;
 ...
 $getidsuggested="SELECT FriendID FROM suggerimenti WHERE mioID=".$ottieniID;
 ...
 $querysugge ="SELECT ID FROM Profiles WHERE Status='Active' AND ID<>".$ottieniID;
 ...
 if ($contasugge>0)
 {
  ...
  $queryverificache = "SELECT Contaric,logrichieste.When, SUBDATE(CURRENT_TIMESTAMP(),1) FROM logrichieste WHERE IdUtente=".$ottieniID;
  ...
  {
   $queryrest="UPDATE logrichieste SET Contaric=0,logrichieste.When=CURRENT_TIMESTAMP WHERE IdUtente=".$ottieniID;
  ...
  if ($allora[0]<$maxnumberoffriendrequests)
  {
   ...
   for ($contatore=0;$contatore<$maxnumberoffriendrequests;$contatore++)
   {
    ...
    $metchpercentage=getProfilesMatch( $ottieniID, $listasugge[0]);
    $mutualfriends=CountMutualFriends ($ottieniID,$listasugge[0]);
    ...
    {
 	   $queryinserta="INSERT INTO suggerimenti (mioID,FriendID,Rifiutato,Pertinenza,MutualF) VALUES(".$ottieniID.",".$listasugge[0].",0,".$metchpercentage.",".$mutualfriends.")";
 	   $resultqueryinserta = mysql_query($queryinserta);
    }
    else
    {
     $arraydegliesclusi=$arraydegliesclusi.",".$listasugge[0];
    }
    
    
   }
   
   $selectsuggestion="SELECT mioID,FriendID,MutualF FROM suggerimenti WHERE (mioID=".$ottieniID." and Rifiutato=0) ORDER BY Pertinenza DESC,MutualF DESC LIMIT ".$limitsuggest;
   ...
   for ($zzetab=0;$zzetab<$miolimite;$zzetab++)  
   {
    ...
    if(is_friends($infosugge['ID'],$ottieniID) or isBlocked($ottieniID,$infosugge['ID'])) 
    { 
     //cancello i suggerimenti di profili che ho già accettato come amici o che ho bloccato
     $removeprofile="DELETE FROM suggerimenti WHERE (FriendID=".$infosugge['ID']." AND mioID=".$ottieniID.") OR (mioID=".$infosugge['ID']." AND FriendID=".$ottieniID.")";
    ...
    else
    {
     ...
     echo '<div class="nomeamico"><a href="'.getProfileLink($infosugge['ID']).'">'.$NomeSugge.'</a></div><div class="affinita">'._t('_ibdw_thirdcolumn_affinita1').' '.getProfilesMatch($ottieniID,$listadeiscelti[1]).'%</div>';
	   if (CountMutualFriends($ottieniID,$listadeiscelti[1])>0) echo "<div class='mtf'>" ._t('_ibdw_thirdcolumn_mutualfriends').' ' . CountMutualFriends($ottieniID,$listadeiscelti[1]) . "</div>";
