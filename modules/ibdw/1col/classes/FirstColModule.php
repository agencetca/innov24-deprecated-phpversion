...
class FirstColModule extends BxDolModule {
    function FirstColModule(&$aModule) {        
        parent::BxDolModule($aModule);
    }

    function actionHome () {
        $this->_oTemplate->pageStart();
        $sDateFormat = getParam ('me_fcol_date_format');
        $isShowUserTime = getParam('me_fcol_enable_js_date') ? true : false;
        $aVars = array (
        ...
        $this->_oTemplate->pageCode(_t('_me_fcol'), true);
    }
    ...
    function actionAdministration () {
	    ...
	    if(empty($iId)) { // if category is not found display page not found
            ...
            $this->_oTemplate->pageCodeAdmin (_t('_me_fcol'));
            return;
        }
        ...
        echo DesignBoxAdmin (_t('_me_fcol'), $sResult); // dsiplay box
        $this->_oTemplate->pageCodeAdmin (_t('_me_fcol')); // output is completed, admin page will be displaed here
    }    
}
?>