...
class MlPageWallTemplate extends BxDolModuleTemplate {
	/**
	 * Constructor
	 */
	function MlPageWallTemplate(&$oConfig, &$oDb) {
	...
	/*function displayBxPhotosAdd($aEvent) {
	    $aOwner = $this->_oDb->getUser($aEvent['owner_id']);
			$sAddedMediaTxt = _t('_page_wall_added_' . $aEvent['media_type']);
  	    return array(
	...
	function displayProfileEdit($aEvent) {
        ...
        if($aOwner['couple'] == 0 && $aOwner['sex'] == 'male')
            $sTxtEditedProfile = _t('_page_wall_edited_his_profile');
        else if($aOwner['couple'] == 0 && $aOwner['sex'] == 'female')
            $sTxtEditedProfile = _t('_page_wall_edited_her_profile');
        else if($aOwner['couple'] > 0)
            $sTxtEditedProfile = _t('_page_wall_edited_their_profile');
	...
	function displayProfileEditStatusMessage($aEvent) {
        ...
        if($aOwner['couple'] == 0 && $aOwner['sex'] == 'male')
            $sTxtEditedProfile = _t('_page_wall_edited_his_profile_status_message');
        else if($aOwner['couple'] == 0 && $aOwner['sex'] == 'female')
            $sTxtEditedProfile = _t('_page_wall_edited_her_profile_status_message');
        else if($aOwner['couple'] > 0)
            $sTxtEditedProfile = _t('_page_wall_edited_their_profile_status_message');
	...
	function displayFriendAccept($aEvent) {
        ...
        return array(
            'title' => $aOwner['username'] . ' ' . _t('_page_wall_friends_with') . ' ' . $aFriend['username'],
            'description' => '',
