...
class MlPageWallDb extends BxDolModuleDb {
    var $_oConfig;
	/*
	 * Constructor.
	 */
	function MlPageWallDb(&$oConfig) {
	...
	function insertPageMedia($sMedia, $iMediaId, $iPageId)
	{
		$sMedia = $sMedia == 'photos' ? 'images' : $sMedia;
		$this->query("INSERT INTO `ml_pages_{$sMedia}` SET `entry_id` = {$iPageId}, `media_id` = {$iMediaId}");
		if ($sMedia == 'images')
			if (!$this->getOne("SELECT `thumb` FROM `ml_pages_main` LEFT JOIN `bx_photos_main` ON `bx_photos_main`.`id` = `ml_pages_main`.`thumb` WHERE `ml_pages_main`.`id` = {$iPageId} LIMIT 1"))
				$this->query("UPDATE `ml_pages_main` SET `thumb` = {$iMediaId} WHERE `ml_pages_main`.`id` = {$iPageId}");
	}
	function getPage($mixed, $sType = 'id') {
	    switch($sType) {
            case 'id':
                $sWhereClause = "`ml_pages_main`.`id`='" . $mixed . "'";
                break;
            case 'uri':
                $sWhereClause = "`ml_pages_main`.`uri`='" . $mixed . "'";
                break;
        } 
	    $sSql = "SELECT `ml_pages_main`.`id` AS `id`, `ml_pages_main`.`uri` AS `entry_uri`, `ml_pages_main`.`title` AS `title`, `ml_pages_main`.`author_id` AS `author_id`, `NickName` AS `username`, `Email` AS `email` FROM `Profiles` 
	    					LEFT JOIN `ml_pages_main` ON `ml_pages_main`.`author_id` = `Profiles`.`id` WHERE " . $sWhereClause . " LIMIT 1";
	    
	    $aUser = $this->getRow($sSql);

	    if(empty($aUser))
	        $aUser = array('id' => 0, 'couple' => 0, 'username' => _t('_page_wall_anonymous'), 'password' => '', 'email' => '', 'sex' => 'male');
	...
	function getEvents($aParams) {
        ...
        switch($aParams['type']) {
            ...
            case 'owner':
                ...
                break;            
            case 'page':
                $sWhereClause = "`page_id`='" . $aParams['page_id'] . "' " . (isset($aParams['filter']) ? $this->_getFilterAddon($aParams['page_id'], $aParams['filter']) : '');
                $sOrderClause = isset($aParams['order']) ? ' ORDER BY `date` ' . strtoupper($aParams['order']) : '';
                $sLimitClause = isset($aParams['count']) ? ' LIMIT ' . $aParams['start'] . ', ' . $aParams['count'] : '';                
                break;  
        }
	    ...
	    while($aEvent) {
            $aEvent['content'] = str_replace("[ray_url]", $sHomeUrl, stripslashes($aEvent['content']));
	...
	function getEventsCount($iPageId, $sFilter) {
	    $sSql = "SELECT COUNT(*) FROM `" . $this->_sPrefix . "events` WHERE `page_id`='" . $iPageId . "'" . $this->_getFilterAddon($iPageId, $sFilter) . " LIMIT 1";
	...
	function _getFilterAddon($iOwnerId, $sFilter) {	    
	    switch($sFilter) {
	        case ML_PAGE_WALL_FILTER_OWNER:
	            $sFilterAddon = " AND `action`='' AND `object_id`='" . $iOwnerId . "' ";
	            break;
	        case ML_PAGE_WALL_FILTER_OTHER:
	            $sFilterAddon = " AND `action`='' AND `object_id`<>'" . $iOwnerId . "' ";
	            break;
	        case ML_PAGE_WALL_FILTER_ALL:
	        default:
	            $sFilterAddon = "";
	    }
	    return $sFilterAddon;
