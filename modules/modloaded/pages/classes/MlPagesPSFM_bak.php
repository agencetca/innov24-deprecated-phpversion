 ...
 */
class MlPagesPSFM {
	function MlPagesPSFM( $iArea ) {
		$this -> aColNames = array (
			1  => array( 'Page' => 'Join',   'Order' => 'JoinOrder',         'Block' => 'JoinBlock',         'ShowSysItems' => 'Couple,Captcha,TermsOfUse,PrimPhoto', 'EditAdd' => array( 'JoinPage' ) ),
			...
			5  => array( 'Page' => 'View',   'Order' => 'ViewAdmOrder',      'Block' => 'ViewAdmBlock',      'ShowSysItems' => 'ID,Date,DateLastEdit,DateLastLogin,Status,Age' ),
			6  => array( 'Page' => 'View',   'Order' => 'ViewMembOrder',     'Block' => 'ViewMembBlock',     'ShowSysItems' => 'ID,Date,DateLastEdit,DateLastLogin,Status,Age' ),
			7  => array( 'Page' => 'View',   'Order' => 'ViewModOrder',      'Block' => 'ViewModBlock',      'ShowSysItems' => 'ID,Date,DateLastEdit,DateLastLogin,Status,Age' ),
			8  => array( 'Page' => 'View',   'Order' => 'ViewVisOrder',      'Block' => 'ViewVisBlock',      'ShowSysItems' => 'ID,Date,DateLastEdit,DateLastLogin,Status,Age' ),
		...
		$this -> sMainCategory = bx_get('main_category');
    if (!$GLOBALS['MySQL']->getOne("SELECT 1 FROM `ml_pages_categories` WHERE `ID`='{$this->sSubCategory}' AND `Parent`='{$this->sMainCategory}' LIMIT 1"))
       $this->sSubCategory = '';
	...
	function fillMyArrays() {
		...
		$sBlocksQuery = "
			...
			WHERE
				...
				`Type` = 'block' AND
				FIND_IN_SET( '{$this -> sSubCategory}', `{$this -> areaPageName}Categories`)
			ORDER BY
				`{$this -> areaOrderCol}`
		";
		...
		while( $aBlock = mysql_fetch_assoc( $rBlocks ) ) {
			...
			$sItemsQuery = "
				...
				WHERE
					...
					FIND_IN_SET( '{$this -> sSubCategory}', `{$this -> areaPageName}Categories`) AND
					(
						`Type` != 'system' OR
						(
							`Type` = 'system' AND
		...
		$sBlocksInacQuery = "
			...
			WHERE
				`Type` = 'block' AND
				(!FIND_IN_SET( '{$this -> sSubCategory}', `{$this -> areaPageName}Categories`) OR `{$this -> areaPageName}Categories` IS NULL)
		";
...
//				) 		
	...
	function savePositions( $aInArrays ) {
		...
		db_res( "UPDATE ml_pages_fields SET `{$this -> areaPageName}Categories` = REPLACE(`{$this -> areaPageName}Categories`,',{$this -> sSubCategory}','')");
		db_res( "UPDATE ml_pages_fields SET `{$this -> areaPageName}Categories` = REPLACE(`{$this -> areaPageName}Categories`,'{$this -> sSubCategory}','')");
		db_res( "UPDATE `ml_pages_fields` SET `{$this -> areaOrderCol}` = NULL" );
		db_res( "UPDATE `ml_pages_fields` SET `{$this -> areaBlockCol}` = 0" );
		...
		if( is_array( $aInArrays['blocks'] ) ) {
			foreach( $aInArrays['blocks'] as $iBlockID ) {
				...
				foreach ($this->aColNames as $iKey => $aValue)
				{
					if ($aValue['Page'] != $this -> areaPageName) continue;

					$iBlockOrd = (int)db_value( "
						SELECT MAX( `{$aValue['Order']}` )
						FROM `ml_pages_fields`
						WHERE `Type` = 'block'
					" ) + 1;
				  switch ($this->iAreaID) {
				       // join
				       case 1:
				           $sAddSet = "SET `JoinOrder` = $iBlockOrd";
				       break;
				       ...
				       case 3:
				       case 4:
				           $sAddSet = "SET `EditOwnOrder` = $iBlockOrd, `EditAdmOrder` = $iBlockOrd, `EditModOrder` = $iBlockOrd";
				       break;
				       ...
				       case 7:
				       case 8:
				           $sAddSet = "SET `ViewAdmOrder` = $iBlockOrd, `ViewMembOrder` = $iBlockOrd, `ViewModOrder` = $iBlockOrd, `ViewVisOrder` = $iBlockOrd";
				       break;
					...
					db_res( "
						UPDATE `ml_pages_fields`
						SET 
						`{$this -> areaPageName}Categories` = CONCAT_WS(',',`{$this -> areaPageName}Categories`,'{$this -> sSubCategory}')
						WHERE `ID` = $iBlockID
					" );
			}
			if( is_array( $aInArrays['items'] ) and is_array( $aInArrays['items_blocks'] ) ) {
				foreach( $aInArrays['items'] as $iItemID ) {
					...
					//`{$aValue['Block']}` = $iItemBlockID
					//`{$aValue['Order']}` = $iItemOrd,
					...
					if( in_array( $iItemBlockID, $aInArrays['blocks'] ) ) {
						foreach ($this->aColNames as $iKey => $aValue)
						{
							$iItemOrd = db_value( "
								SELECT MAX( `{$aValue['Order']}` )
								FROM `ml_pages_fields`
								WHERE `Type` != 'block' AND `{$aValue['Block']}` = $iItemBlockID
							" ) + 1;
						  switch ($this->iAreaID) {
						       // join
						       case 1:
						           $sAddSet = "SET `JoinOrder` = $iItemOrd, `JoinBlock` = $iItemBlockID";
						       break;
						       ...
						       case 3:
						       case 4:
						           $sAddSet = "SET `EditOwnOrder` = $iItemOrd, `EditAdmOrder` = $iItemOrd, `EditModOrder` = $iItemOrd, `EditOwnBlock` = $iItemBlockID
						           	,`EditAdmBlock` = $iItemBlockID, `EditModBlock` = $iItemBlockID";
						       break;
						       ...
						       case 7:
						       case 8:
						           $sAddSet = "SET `ViewAdmOrder` = $iItemOrd, `ViewMembOrder` = $iItemOrd, `ViewModOrder` = $iItemOrd, `ViewVisOrder` = $iItemOrd
						           	,`ViewAdmBlock` = $iItemBlockID, `ViewMembBlock` = $iItemBlockID, `ViewModBlock` = $iItemBlockID, `ViewVisBlock` = $iItemBlockID";
						       break;
	...
	function genFieldEditForm( $iItemID ) {
		$sQuery = "
			...
			FROM `ml_pages_fields` WHERE `ID` = $iItemID";
		...
		if( $aField['Type'] == 'block' ) {
			if( $this -> iAreaID == 5 || $this -> iAreaID == 6 || $this -> iAreaID == 7 || $this -> iAreaID == 8) { //Join
				?>
			<tr>
				<td class="label"><?= _t('_ml_pages_box_height') ?>:</td>
				<td class="value">
						<input type="text" name="BoxHeight" value="<?=$aField['BoxHeight']?>" class="input_text"/>
				</td>
				<td class="info">
					<?= $this -> getInfoIcon( _t('_ml_pages_box_height_desc')); ?>
				</td>
			</tr>
				<?
			}
	...
	function saveItem( $aData ) {
		...
		if( $aItem['Type'] == 'block' and $this -> iAreaID == 1 ) { //Join
			//get JoinPage
			$aUpdate['BoxHeight'] = (int)$aData['BoxHeight'];
	...
	function createNewField() {
		...
		if( db_res( "INSERT INTO `ml_pages_fields` (`Name`, `Type` ) VALUES ('NEW_ITEM', 'text')", 0 ) and $iNewID = db_last_id() ) {
			...
			if( !db_res( "ALTER TABLE ml_pages_main ADD `NEW_ITEM` varchar(255) NOT NULL default ''", 0 ) ) {
				...
				db_res( "DELETE FROM `ml_pages_fields` WHERE `ID` = $iNewID" );
				$iNewID = 0;
			}
		}
	...
	function createNewBlock() {
		db_res( "INSERT INTO `ml_pages_fields` (`Name`, `Type` ) VALUES ('NEW BLOCK', 'block')", 0 );
		$iNewID = db_last_id();
	...
	function deleteItem( $iItemID ) {
		...
		elseif( $aItem['Type'] == 'system' or !(int)$aItem['Deletable'] )
			...
			if( $aItem['Type'] == 'block' )
			{
				...
				db_res( "UPDATE `ml_pages_fields` SET `JoinCategories` = '', `ViewCategories` = '', `EditCategories` = '' WHERE `EditOwnBlock` = '$iItemID' AND `JoinBlock` = '$iItemID' AND `ViewMembBlock` = '$iItemID'" );
			}
			if( $aItem['Type'] != 'block' )
...
class BxDolPSFMCacher {
	...
	function BxDolPSFMCacher() {
		...
		$this -> aAreasProps = array (
			1  => array( 'Title' => 'Join',            'Order' => 'JoinOrder',         'Block' => 'JoinBlock',         'AddSelect' => 'Control,Extra,Min,Max,Values,Check,Unique,Mandatory,UseLKey,Default,Multiplyable,WithPhoto', 'AddBlockProps' => $aAddBlockProps['Join'],   'AddProps' => $aAddProps['Join']   ),
			2  => array( 'Title' => 'Edit (Owner)',    'Order' => 'EditOwnOrder',      'Block' => 'EditOwnBlock',      'AddSelect' => 'Control,Extra,Min,Max,Values,Check,Unique,Mandatory,UseLKey,Multiplyable,WithPhoto', 'AddBlockProps' => $aAddBlockProps['Edit'],   'AddProps' => $aAddProps['Edit']   ),
			3  => array( 'Title' => 'Edit (Admin)',    'Order' => 'EditAdmOrder',      'Block' => 'EditAdmBlock',      'AddSelect' => 'Control,Extra,Min,Max,Values,Check,Unique,Mandatory,UseLKey,Multiplyable,WithPhoto', 'AddBlockProps' => $aAddBlockProps['Edit'],   'AddProps' => $aAddProps['Edit']   ),
			4  => array( 'Title' => 'Edit (Moder)',    'Order' => 'EditModOrder',      'Block' => 'EditModBlock',      'AddSelect' => 'Control,Extra,Min,Max,Values,Check,Unique,Mandatory,UseLKey,Multiplyable,WithPhoto', 'AddBlockProps' => $aAddBlockProps['Edit'],   'AddProps' => $aAddProps['Edit']   ),
			
			5  => array( 'Title' => 'View (Admin)',    'Order' => 'ViewAdmOrder',      'Block' => 'ViewAdmBlock',      'AddSelect' => 'Values,UseLKey,Multiplyable,WithPhoto,CaptionDisable,MultipleStyle,MultipleCss,Link', 'AddBlockProps' => $aAddBlockProps['View'],                                                'AddProps' => $aAddProps['View']   ),
			6  => array( 'Title' => 'View (Member)',   'Order' => 'ViewMembOrder',     'Block' => 'ViewMembBlock',     'AddSelect' => 'Values,UseLKey,Multiplyable,WithPhoto,CaptionDisable,MultipleStyle,MultipleCss,Link',                                              'AddBlockProps' => $aAddBlockProps['View'],   'AddProps' => $aAddProps['View']   ),
			7  => array( 'Title' => 'View (Moder)',    'Order' => 'ViewModOrder',      'Block' => 'ViewModBlock',      'AddSelect' => 'Values,UseLKey,Multiplyable,WithPhoto,CaptionDisable,MultipleStyle,MultipleCss,Link',                                              'AddBlockProps' => $aAddBlockProps['View'],   'AddProps' => $aAddProps['View']   ),
			8  => array( 'Title' => 'View (Visitor)',  'Order' => 'ViewVisOrder',      'Block' => 'ViewVisBlock',      'AddSelect' => 'Values,UseLKey,Multiplyable,WithPhoto,CaptionDisable,MultipleStyle,MultipleCss,Link',                                              'AddBlockProps' => $aAddBlockProps['View'],   'AddProps' => $aAddProps['View']   ),
	...
	function createOldCache() {
		...
		foreach ($this -> aAreasProps as $iAreaID => $aArea) {
			$oArea = new BxDolPageFieldsArea( $iAreaID, $this );
...
class BxDolPageFieldsArea {
	...
	function BxDolPageFieldsArea( $iAreaID, &$oParent ) {
		$this -> id = $iAreaID;
	...
	function getBlocksCacheString( $sPrefix = '' ) {
		...
		foreach ($this -> aBlocks as $iBlockID => $sBlockName) {
			...
			$aCategories = db_arr("SELECT `JoinCategories`, `EditCategories`, `ViewCategories`, `SearchCategories`, `BoxHeight` FROM `ml_pages_fields` WHERE `Name` = '{$sBlockName}' LIMIT 1");
			...
			if( is_array($this -> aAddBlockProps) )
				foreach ($this -> aAddBlockProps as $sProp => $sValue) {
					...
					{
						case 'JoinCategories':
							$sPropValue = str_replace( '{JoinCategories}', $aCategories['JoinCategories'], $sValue );
						break;
						case 'EditCategories':
							$sPropValue = str_replace( '{EditCategories}', $aCategories['EditCategories'], $sValue );
						break;
						case 'ViewCategories':
							$sPropValue = str_replace( '{ViewCategories}', $aCategories['ViewCategories'], $sValue );
						break;
						case 'SearchCategories':
							$sPropValue = str_replace( '{SearchCategories}', $aCategories['SearchCategories'], $sValue );
						break;
						case 'BoxHeight':
							$sPropValue = str_replace( '{BoxHeight}', $aCategories['BoxHeight'], $sValue );
						break;
					}
