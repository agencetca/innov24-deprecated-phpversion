...
class MlPagesPageFields extends Thing {
    ...
    function MlPagesPageFields( $iAreaID, $sMainCategory = '', $sSubCategory ='' ) {
    		global $site;
        ...
        switch($this -> iAreaID)
        {
        	...
        	case 10 : 
        	case 11 :
        		$this -> areaPageName = 'Search';
        	break;
        }
        //$this->sCategory = 'Amateur-Sports-Team';
        $this->sMainCategory = $_POST['main_category'] ? $_POST['main_category'] : $this -> sMainCategory;
       	...
       	//if (!$GLOBALS['MySQL']->getOne("SELECT 1 FROM `ml_pages_pre_values` WHERE `Key`='{$this->sMainCategory}' AND `Value`='{$this->sSubCategory}' LIMIT 1"))
    ...
    function processPostValues( $bCouple, &$aValues, &$aErrors, $iPage = 0, $iProfileID = 0, $iBlockOnly = 0 ) {
        ...
        foreach( $this -> aBlocks as $iBlockID => $aBlock ) {
        		if (!$aBlock[$this -> areaPageName.'Categories']) continue;
        		$aCategories = explode(',', $aBlock[$this -> areaPageName.'Categories']);
...
//print_r( $_FILES).'<br>';
            ...
            if (!$aItem[$this -> areaPageName.'Categories']) continue;
        		$aCategories = explode(',', $aItem[$this -> areaPageName.'Categories']);
        		if (!in_array($this->sSubCategory, $aCategories)) continue;
                ...
                for( $iHuman = 0; $iHuman < $iHumans; $iHuman ++ ) {
                    ...
                    switch( $aItem['Type'] ) {
                        ...
                        case 'pass':
                        case 'select_one':
                        		if ($aItem['Multiplyable'] && !empty($_POST[$sItemName]))
                        		{
                        			foreach($_POST[$sItemName] as $sKey => $sValue)
                        				$aProcValues[$sKey] = process_pass_data($sValue);
                        			$mValue = implode(getParam('ml_pages_multi_divider'), $aProcValues);
                        		}
											             ...
											             if (move_uploaded_file($_FILES[$aItem['Name'] . '_photo']['tmp_name'][$i], $sTmpName))
											               $mValuePhoto[$i] = basename($sTmpName);
															...
															if (!empty($mValuePhoto))
																$mValuePhotos = implode(getParam('ml_pages_multi_divider'), $mValuePhoto);
														}
                        break;
                        case 'html_area':
                        ...
                        case 'system':
                            switch( $aItem['Name'] ) {
                                ...
                                case 'PrimPhoto':
                                    if (isset($_FILES['PrimPhoto'])) {
                                        if ($_FILES['PrimPhoto']['error'] == UPLOAD_ERR_OK) {
                                            $sTmpName  = tempnam($GLOBALS['dir']['tmp'], 'pphot');
                                            if (move_uploaded_file($_FILES['PrimPhoto']['tmp_name'], $sTmpName))
                                                $mValue = basename($sTmpName);
                                        }
                                    } elseif (isset($_POST['PrimPhoto']) && trim($_POST['PrimPhoto'])) {
                                        $mValue = preg_replace('/[^a-zA-Z0-9\.]/', '', $_POST['PrimPhoto']);
                                    }
                                break;
                            }
                        break;
                    ...
                    $rRes = $this -> checkPostValue( $iBlockID, $iItemID, $mValue, $iHuman, $iProfileID, $mValuePhoto );
                    
                    if( $rRes !== true && !$aItem['WithPhoto'])
    ...
    function checkPostValue( $iBlockID, $iItemID, $mValue, $iHuman, $iProfileID, $mValuePhoto = array() ) {
        // get item
        ...
        $aChecks = array(
            'text' => array( 'Mandatory', 'Min', 'Max', 'Unique', 'Check', 'WithPhoto' ),
            'area' => array( 'Mandatory', 'Min', 'Max', 'Unique', 'Check', 'WithPhoto' ),
        ...
        foreach ($aMyChecks as $sCheck ) {
            ...
            $mRes = $this -> $sFunc( $aItem, $mValue, $iHuman, $iProfileID, $mValuePhoto );
		...
		function checkPostValueForWithPhoto($aItem, $mValue, $iHuman, $iProfileID, $mValuePhoto) 
		{
            ...
            case 'text':
	                ...
	                if ($aItem['Link'])
	                {
	                	...
	                	$sHttp = $aUrl['scheme'] == 'http' ? '' : 'http://';
	                 	return '<a target="_blank" href="' . $sHttp . $sValue.'">' . nl2br($sValue) . '</a>';
	                }
            			return nl2br(($sValue));
            		}
	              if ($aItem['Link'])
	              {
	              		$aUrl = parse_url($sValue);
	                	$sHttp = $aUrl['scheme'] == 'http' ? '' : 'http://';
	                 return '<a target="_blank" href="'. $sHttp . $sValue.'">' . nl2br(htmlspecialchars_adv($sValue)) . '</a>';        		
                }
        ...
        foreach ($this->aBlocks as $iBlockId => $aBlock) {
            ...
            foreach ($aBlock['Items'] as $iItemId => $aItem) {
                ...
                $aFormInput = array(
                    ...
                    'value'   => $aDefaultParams[$aItem['Name']],
                );
                ...
                switch ($aItem['Type']) {
                    ...
                    case 'select_set':
                        ...
                        if ($aItem['Name'] == 'Country') 
                        {
                        	$aFormInput['value'] = $aDefaultParams['Country'] = '';
                        	$aFormInput['values'] = array_merge(array('All'), $aFormInput['values']);
                        }
                        //* scriptologist
                            ...
                            case 'Location':
                                ...
                                $aFormInput['content'] = <<<EOF
                                    <div class="location_wrapper">
                                        <div>
                                            <input type="text" name="distance" class="form_input_text form_input_distance" />
                                        ...
                                        <div>
                                            $sFromZipC
                                            <input type="text" name="zip" class="form_input_text form_input_zip" />
                                        </div>
                                    </div>                            
EOF;
                            break;
    ...
    function getFormJoin($aParams) {
        ...
        */
        ...
        foreach( $this->aArea[$iPage] as $aBlock ) {
        		if (!$aBlock['JoinCategories'] || !$this->sSubCategory) continue;
        		$aCategories = explode(',', $aBlock['JoinCategories']);
        		if (!in_array($this->sSubCategory, $aCategories)) continue;
            ...
            // add every item
            foreach( $aBlock['Items'] as $aItem ) {
            		if (!$aItem['JoinCategories']) continue;
		        		$aCategories = explode(',', $aItem['JoinCategories']);
		        		if (!in_array($this->sSubCategory, $aCategories)) continue;
    ...
    function getFormEdit($aParams) {
        ...
        */
        ...
        // add every block on this page
        foreach( $this->aBlocks as $aBlock ) {
        		if (!$aBlock['EditCategories'] || !$this->sSubCategory) continue;
        		$aCategories = explode(',', $aBlock['EditCategories']);
        		if (!in_array($this->sSubCategory, $aCategories)) continue;
            // generate block header
            $aInputs[] = array(
                'type' => 'block_header',
            ...
            // add every item
            foreach( $aBlock['Items'] as $aItem ) {
            		if (!$aItem['EditCategories'] || !$this->sSubCategory) continue;
		         		$aCategories = explode(',', $aItem['EditCategories']);
		        		if (!in_array($this->sSubCategory, $aCategories)) continue;               
                $aInputParams = array(
                    'values'         => array(
                        0 => isset($aValues[0][$aItem['Name']]) ? $aValues[0][$aItem['Name']] : null,
        ...
        $aForm = array(
            'form_attrs' => array(
                'name'     => 'edit_form',
                'action'   => BX_DOL_URL_ROOT . 'modules/modloaded/pages/edit.php?page_id=' . $iPageID,
                'method'   => 'post',
                'enctype'  => 'multipart/form-data',
                'onsubmit' => 'return validateJoinForm(this);',
            ),
    ...
    function convertEditField2Input($aItem, $aParams, $iPerson) {
        ...
        if ($aItem['WithPhoto'])
        {
					...
					{
						case 'text' :
							$sInput = '<div class="input_wrapper input_wrapper_text">
							            <input class="form_input_text" type="text" name="'.$aItem['Name'].'[]"/>
							            <div class="input_close input_close_text"></div>
							        </div>';
						break;
						case 'area' :
             $sInput = '<div class="input_wrapper input_wrapper_textarea" >
                         	<div class="input_border">
                          	<textarea  counter="true" min="'.$aItem['Min'].'" class="form_input_textarea" name="'.$aItem['Name'].'[]"></textarea>
													</div>
          ...
          if ($aItem['Multiplyable'])
          	$aTemplates[$aItem['Name']]['upload'] = $oMlPages->_oTemplate->parseHtmlByName('form_field_files_upload_multiple', $aVarsUpload);
          else
          	$aTemplates[$aItem['Name']]['upload'] = $oMlPages->_oTemplate->parseHtmlByName('form_field_files_upload', $aVarsUpload);
					$aInput['type'] = 'custom';
					$aInput['content'] = $aTemplates[$aItem['Name']]['upload'];
        }
        if ($aItem['Multiplyable'])
        {
...
BLAH;
    ...
    function convertJoinField2Input($aItem, $aParams, $iPerson) {
        ...
        if ($aItem['Multiplyable'])
        {
        	$aInput['value'] = explode(getParam('ml_pages_multi_divider'), $aInput['value']);
        	$aInput['name'] = $aItem['Name'] . '[]';
        	$aInput['attrs']['multiplyable'] = 'true';
        } 
