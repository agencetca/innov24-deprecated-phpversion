...
class MlPagesController {
	...
	function createPage( $aData, $bSendMails = true, $iMainMemberID = 0 ) {
		...
		$sEntryUri = uriGenerate($aNewPage['title'], 'ml_pages_main', 'uri');
        ...
        if( $rRes ) {
			...
			$iCategory = bx_get('sub_category');
			$sStatus = getParam('ml_pages_autoapproval') == 'on' || isAdmin() ? 'approved' : 'pending';
			$this -> updatePage($iNewID, array(
                'created' => time(),
                'uri' => $sEntryUri,
                'categories' => db_value("SELECT `Name` FROM `ml_pages_categories` WHERE `ID` = {$iCategory} LIMIT 1"),
                'category' => $iCategory,
            ));            			
	...
	function createPageDataFile( $iPageID ) {
		...
		$iPageID = (int)$iPageID;
		$fileName = BX_DIRECTORY_PATH_CACHE . 'pages' . $iPageID . '.php';
		if( $userID > 0 ) {
