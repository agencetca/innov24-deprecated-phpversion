<?php
    ...
    ***************************************************************************/
	...
	if($aMailBoxSettings['mailbox_mode'] == 'view_message'){
		//[begin] credits mod (modzzz.com)  
		require_once(BX_DIRECTORY_PATH_CLASSES . 'BxDolAlerts.php');
		$oAlert = new BxDolAlerts('mail', 'view', $vRecipientID, $iProfileId);
		$oAlert->alert();  
    		...
    		case 'reply_message' :
    			if ( $iMessageID and $vRecipientID )
    			{
					...
					if(getParam("modzzz_credit_activated")){
						...
						$sCode = $oCredit->_oDb->checkSendMailAccess($iProfileId);
						if($sCode){   
							$sOutputHtml = $sCode; 
						}else{
							$sOutputHtml = $oMailBox -> genReplayMessage($vRecipientID, $iMessageID);
    ...
    $_page['js_name']  = 'mail_box.js';

    $GLOBALS['oTopMenu'] -> setCustomSubIconUrl( getTemplateIcon('bx_mymail.png') ); 
	...
	//[begin] credits mod (modzzz.com) 
	if(getParam("modzzz_credit_activated") && $aMailBoxSettings['mailbox_mode']=='compose'){
		$oCredit = BxDolModule::getInstance('BxCreditModule');  
		$sCode = $oCredit->_oDb->checkSendMailAccess($iProfileId);
		if($sCode){  
